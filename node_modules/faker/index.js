...
const UserAccountJson = {
    insertedAccountNos : insertedAccountNos,
    insertedUserNames : insertedUserNames
};
...
async function insertMemberData() {
    ...
    for (let i = 0; i < numOfRows; i++) {
        ...
        const loginId = faker.internet.email();
...
async function insertApiAccountDetailData() {
    ...
    for (let i = 0; i < numOfRows; i++) {
        ...
        // 최종수취고객 컬럼 추가에 따른 추가
        if(inoutTypeCd == 'O'){
            recvClientAccountNum = faker.datatype.number({ min: 1000000000, max: 99999999999999 }).toString();

            const probabilityWithValue = 0.2; // 예적금 수취인 값을 넣을 확률 (20%)

            for (let i = 0; i < numOfRows; i++) {
                if (inoutTypeCd === 'O' && Math.random() < probabilityWithValue) {
                    //지정한 확률로 값을 넣습니다.
                    const randomIdx = faker.datatype.number({ min: 0, max: insertedUserNames.length - 1 });
                    recvClientAccountNum = insertedAccountNos[randomIdx];
                }
            }
        ...
        let tranAmt = 0;
        if(inoutTypeCd == 'O'){
             tranAmt = faker.datatype.number({ min: 1, max: 1000000 });
        }else{
            // inoutTypeCd가 'I'일 시, 연소득으로 계산 , 연소득의 경우 데이터가 지저분해질 수 있어 다음과 같이 정함(최저시급 계산)
            faker.random.arrayElement(0, 500, 1000, faker.datatype.number({ min: 24730000, max: 2000000000 }));
        }
